...
const ffmpeg = require('bluebird').promisifyAll(require("fluent-ffmpeg"));
...
export default class Video {
    ...
    async initialize() {
        ...
        let metadata = await ffmpeg.ffprobe(this.input.path);
    ...
    stop(err) {
        ...
        let stats = Promise.all([fs.stat(this.input.path), fs.stat(this.output.path)]);
